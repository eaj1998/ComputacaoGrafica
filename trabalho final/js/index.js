	...
	loaderFBX.load(
		...
		function (obj) {
			...
			anim.load("Walking.fbx", (anim) => {
				...
				const kick = mixer1.clipAction(anim.animations[0]);
				kick.play();
			});
	...
	loaderFBX.load(
		...
		function (obj) {
			...
			anim.load("Walking.fbx", (anim) => {
				...
				const kick = mixer2.clipAction(anim.animations[0]);
				kick.play();
			});
	...
	loaderFBX.load(
		...
		function (obj) {
			...
			anim.load("dancing.fbx", (anim) => {
				...
				const kick = mixer3.clipAction(anim.animations[0]);
				kick.play();
			});
	...
	loaderFBX.load(
		...
		function (obj) {
			...
			anim.load("Terrified.fbx", (anim) => {
				mixerPessoa2 = new THREE.AnimationMixer(obj);
				const kick = mixerPessoa2.clipAction(anim.animations[0]);
				kick.play();
			});
...
var createGui = function () {
	...
	comboChange.onChange(function (parametro) {
		if (parametro == 'Zombie') {
			camera.lookAt(elementos["zombie3"].position);
			parametrosGUI.modelGui = "zombie";
		} else if (parametro == 'Pessoa') {
			camera.lookAt(elementos["pessoa2"].position);
			parametrosGUI.modelGui = "pessoa2";
		}
	}
	);
...
var animation = function () {
	...
	if (loadFinishedPessoa1) {
		mixerPessoa2.update(delta);
	} else {
		console.log("Não!!!");
	}

	if (loadFinishedPessoa2) {
		mixerPessoa2.update(delta);
	} else {
		console.log("Não!!!");
	}
